#!/usr/bin/python3
'''The solving nqueens problem'''
import sys


def is_valid(board, row, col):
    """
    Check if a position of queen is valid
    Args:
        board: 2D array representing board
        row: row of queen
        col: column of queen
    Returns:
...
def nqueens_helper(board, col):
    """
    Helper of function for nqueens
    Args:
        board: 2D array representing board
        col: column to start from
    Returns:
        Boolean: True if a solution is found, False other than that
    """
    if col >= len(board):
